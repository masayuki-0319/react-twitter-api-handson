  ...
  render() {
    return (
      <div className='flex m-1'>
        {createRaneItems(this.state.raneNum, this.props.images).map(
          (items: typeRaneItems[], index: number) => {
            return (
              <div key={index}>
...
function createRaneItems(
  rane_num: number,
  items: typeItems
): typeRaneItems[][] {
  const RaneItems: typeRaneItems[][] = Array(rane_num)
    .fill([])
    .map((_i) => []);
